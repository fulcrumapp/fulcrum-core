...
import { DynamicElementsElement, DynamicElementsValue, CheckBoxElement, CheckBoxValue } from '../../src';
...
describe('dynamic elements fields', () => {
  it('finds a dynamic elements field in the form', () => {
    ...
    field.should.be.instanceof(DynamicElementsElement);
  });

  it('finds a dynamic elements value in the record', () => {
    const value = record.formValues.find('checklist');

    value.should.be.instanceof(DynamicElementsValue);
    ...
    const cbValue = value.items[0].formValues.get(cbElement.key);
