...
- [x] 2.1 Add PhotoElement unit coverage for all three supported modes, missing/null input, unsupported modes, malformed target arrays, large valid target arrays, opaque keys, order preservation, and defensive-copy behavior.
- [x] 2.2 Run `yarn test`, `yarn lint`, and `yarn build`; resolve regressions without broadening the change.
